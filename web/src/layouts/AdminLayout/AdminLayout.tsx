...
const AdminLayout = ({ buttonLabel, buttonTo, children }: LayoutProps) => {
  const navigation = [
    {
      href: routes.admin(),
      name: "Admin",
          ...
          <ul className="flex w-full items-center gap-x-3 overflow-x-auto border-b">
            {navigation.map((item, idx) => (
              ...
              >
                <a
                  href={item.href}
                  className="rounded-lg px-4 py-2.5 text-sm font-medium duration-150 hover:bg-gray-50 hover:text-purple-600 active:bg-gray-100"
                >
                  {item.name}
                </a>
              </li>
